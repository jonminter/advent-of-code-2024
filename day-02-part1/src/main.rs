...
fn determine_if_report_safe(maybe_report: Result<Vec<i32>, Error>) -> Result<bool, Error> {
    maybe_report.map(|report| {
        ...
        let all_increasing = all_diffs
            .iter()
            .all(|d| *d > 0);
        let all_decreasing = all_diffs
            .iter()
            .all(|d| *d < 0);
